% Plot estimation errors with respect to s 
close all
clear all

alpha = 1; % 0 <= alpha <= 1
n = 100;      % numbe of labellers, natural number >= 3
s = 100;      % number of tasks, natural number
% q= ones(3,1);
q = [0.25 , 0.3 , 0.4];
for j = 1:3
    ...
    %% Methode 3
     p = [(2*q(j)-1/2) * ones(n/2,1) ; 1/2 * ones(n/2,1) ];
...
figure (3)
title('Performance of the $\hat{G(t)}$', 'interpreter','latex')
xlabel('Number of tasks t')
ylabel('$\hat{G(t)} - G(t)$', 'interpreter','latex')